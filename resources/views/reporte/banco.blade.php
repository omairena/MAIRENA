@extends('layouts.app', ['page' => __('Reporte de Egresos Bancarios en Excel'), 'pageSlug' => 'reportesCxc'])
@section('content')
<div class="row">
        <div class="col-md-12">
            <div class="card">
                ...
                <div class="card-body">   
                    <form method="post" action="{{ route('filtro.bancos') }}" autocomplete="off" id="filtro_factura">
                    @csrf
...
<script type="text/javascript">
    $(document).ready(function() {
            ...
            var APP_URL = {!! json_encode(url('/consultaEmpresad')) !!};
            $.ajax({
                ...
                success:function(response){
                    ...
                    if (arreglo > 0) {
                        ...
                        $("#actividad").append('<option value="'+response['success']['codigo_actividad'][data].id_bancos+'">'+ response['success']['codigo_actividad'][data].id_bancos+' - '+ response['success']['codigo_actividad'][data].cuenta+'</option>');
                        });
                    }else{
                        $('#actividad').find('option').remove();
