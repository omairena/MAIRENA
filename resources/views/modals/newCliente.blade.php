<div class="modal fade" id="newUsuario" tabindex="-1" role="dialog" style="display: none;" aria-hidden="true">
  <div class="modal-dialog modal-lg-inventario" role="document">
    <div class="modal-content">
      <form method="post" action="{{ route('cliente.jsonstore') }}" autocomplete="off" enctype="multipart/form-data" id="form_new_cliente">
        @csrf
  ...
  <div class="form-group{{ $errors->has('codigo_actividad_modal') ? ' has-danger' : '' }}">
    ...
    <!-- Input con datalist para permitir escritura libre -->
    <input type="text" list="actividad-list-modal" id="codigo_actividad_modal" name="codigo_actividad_modal" class="form-control form-control-alternative" value="{{ old('codigo_actividad_modal', $cliente->codigo_actividad_modal ?? '') }}" placeholder="Escribe o elige un código" required>
    <datalist id="actividad-list-modal">
