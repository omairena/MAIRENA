        ...
        <div class="row">
            <div class="col-xl-12 order-xl-1">
                <div class="card">
                    ...
                    <div class="card-body">
                        <form method="post" action="{{ route('list.update', $lista->idlist) }}" autocomplete="off" enctype="multipart/form-data">
                            @csrf
                            @method('PUT')
                            ...
                            <div class="form-group{{ $errors->has('descripcion') ? ' has-danger' : '' }}">
                                ...
                                <input type="text" name="descripcion" id="descripcion" class="form-control form-control-alternative{{ $errors->has('descripcion') ? ' is-invalid' : '' }}" placeholder="{{ __('Descripcion de la Lista') }}" value="{{ old('descripcion', $lista->descripcion) }}" required>
                            ...
                            <div class="form-group{{ $errors->has('porcentaje') ? ' has-danger' : '' }}">
                                ...
                                <input type="text" name="porcentaje" id="porcentaje" class="form-control form-control-alternative{{ $errors->has('porcentaje') ? ' is-invalid' : '' }}" placeholder="{{ __('Porcentaje de la Lista') }}" value="{{ old('porcentaje', $lista->porcentaje) }}" required>
                            ...
                            <div class="form-group{{ $errors->has('estatus') ? ' has-danger' : '' }}">
                                ...
                                <select class="form-control form-control-alternative" id="estatus" name="estatus" value="{{ old('estatus') }}" required>
                                    <option value="1" {{ ($lista->estatus == 1 ? 'selected="selected"' : '') }}>Activo</option>
                                    <option value="0" {{ ($lista->estatus == 0 ? 'selected="selected"' : '') }}>Inactivo</option>
                                </select>
