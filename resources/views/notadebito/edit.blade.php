        ...
        <div class="row">
            <div class="col-xl-12 order-xl-1">
                <div class="card">
                    ...
                    <div class="card-body">
                        <form method="post" action="{{ route('notadebito.update', $sales->idsale) }}" autocomplete="off" enctype="multipart/form-data">
                            @csrf
                            @method('PUT')
                            ...
                            <div class="pl-lg-4">
                                ...
                                <div class="table-responsive">
    								<table class="table align-items-center" id="tabla_devolucion">
    									...
    									<tbody class="tabla_productos">
                                             ...
                                             ?>
                                             @foreach($sales_item_detalle as $sale_it)
                                            <?php
                                                if ($sale_it->existe_exoneracion == '00') {
                                                        $tiene_exoneracion = 'No';
                                                        $total = ($sale_it->valor_neto - $sale_it->valor_descuento)+ $sale_it->valor_impuesto;
                                                        $total_impuesto = $total_impuesto + $sale_it->valor_impuesto;
                                                    }else{
                                                        $exoneracion = App\Items_exonerados::where('idsalesitem', $sale_it->idsalesitem)->get();
                                                        $tiene_exoneracion = 'Si '.$exoneracion[0]->porcentaje_exoneracion. ' %';
                                                        $monto_imp_exonerado = $sale_it->valor_impuesto - $exoneracion[0]->monto_exoneracion;
                                                        $total = ($sale_it->valor_neto - $sale_it->valor_descuento)+ $monto_imp_exonerado;
                                                        $total_impuesto = $total_impuesto + $monto_imp_exonerado;
                                                    }
                                                    $total_neto = $total_neto + $sale_it->valor_neto;
                                                    $total_descuento = $total_descuento + $sale_it->valor_descuento;

                                                    $total_comprobante = $total_comprobante + $total;

                                            ?>
                                            <tr>
                                                    <td>{{ $sale_it->codigo_producto }}</td>
                                                    <td>{{ $sale_it->nombre_producto }}</td>
                                                    <td>{{ $sale_it->costo_utilidad }}</td>
                                                    @if($sale_it->existe_exoneracion === '00')
                                                    <td class="text-right"><input type="number" name="cantidad" id="cantidad{{ $sale_it->idsalesitem }}" value="{{ $sale_it->cantidad }}" class="form-control form-control-alternative{{ $errors->has('cantidad') ? ' is-invalid' : '' }} update_cantidad_factura" style="width:80px;" data-id="{{ $sale_it->idsalesitem }}" data-producto="{{ $sale_it->idproducto }}"></td>
                                                    <td class="text-right"><input type="number" name="descuento" id="descuento{{ $sale_it->idsalesitem }}" value="{{ $sale_it->descuento_prc }}" class="form-control form-control-alternative{{ $errors->has('descuento') ? ' is-invalid' : '' }} update_descuento_factura" style="width:80px;" data-id="{{ $sale_it->idsalesitem }}" data-producto="{{ $sale_it->idproducto }}"></td>
                                                    @else
                                                    <td class="text-right"><input type="number" name="cantidad" id="cantidad{{ $sale_it->idsalesitem }}" value="{{ $sale_it->cantidad }}" class="form-control form-control-alternative{{ $errors->has('cantidad') ? ' is-invalid' : '' }} update_cantidad_factura" style="width:80px;" data-id="{{ $sale_it->idsalesitem }}" data-producto="{{ $sale_it->idproducto }}"></td>
                                                    <td class="text-right"><input type="number" name="descuento" id="descuento{{ $sale_it->idsalesitem }}" value="{{ $sale_it->descuento_prc }}" class="form-control form-control-alternative{{ $errors->has('descuento') ? ' is-invalid' : '' }} update_descuento_factura" style="width:80px;" data-id="{{ $sale_it->idsalesitem }}" data-producto="{{ $sale_it->idproducto }}"></td>
                                                    @endif
                                                    <td>{{ $sale_it->valor_neto }}</td>
                                                    <td>{{ $sale_it->valor_descuento }}</td>
                                                    @if($sale_it->existe_exoneracion === '00')
                                                    <td>{{ $sale_it->valor_impuesto }}</td>
                                                    @else
                                                    ...
                                                    <td>
                                                                            <div class="dropdown">
                                                                                ...
                                                                                <div class="dropdown-menu dropdown-menu-right dropdown-menu-arrow">
                                                                                    ...
                                                                                    @endif
                                                                                    @if($sale_it->prod_sale[0]->flotante > 0)
                                                                                    <a href="#" id="modificar_articulo_flotante{{ $sale_it->idsalesitem }}" data-id="{{ $sale_it->idsalesitem }}" class="dropdown-item modificar_flotante" data-target="#ModArticulo" data-toggle="modal">{{ __('Modificar Artículo') }}</a>
                                ...
                                <div class="col-12">
                                    <div class="form-group{{ $errors->has('observaciones') ? ' has-danger' : '' }}">
                                        <label class="form-control-label" for="observaciones">{{ __('Observaciones de Nota de Debito') }}</label>
                                        <textarea id="observaciones" name="observaciones" class="form-control" required="true">{{ $sales->observaciones }}</textarea>
                                    </div>
                                </div>
                                <div class="text-center">
                                    <button type="submit" class="btn btn-success mt-4" id="guardar_factura">{{ __('Guardar') }}</button>
                                </div>
                            </div>
                        </form>
                    </div>
