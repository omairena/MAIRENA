@extends('layouts.app', ['page' => __('Reporte de Cuentas por Pagar'), 'pageSlug' => 'reportesCxp'])
@section('content')
<div class="row">
        <div class="col-md-12">
            <div class="card">
                ...
                <div class="card-body">   
                    <form method="post" action="{{ route('filtro.cxp') }}" autocomplete="off" id="filtro_factura">
                    @csrf
                            <label class="form-control-label" for="input-fecha_desde">{{ __('Selección de Cuenta por Pagar') }}</label>
                            <select class="form-control form-control-alternative" id="idcxpagar" name="idcxpagar" value="{{ old('idcxpagar') }}" required>
                                <option value="0">-- Seleccionar Todas las cuentas --</option>
                                @foreach($cxpagar as $cxp) 
                                        <option value="{{ $cxp->idcxpagar }}">{{ $cxp->idcxpagar }} - {{ $cxp->cxpagar_cli[0]->nombre }} - {{ number_format($cxp->saldo_pendiente,2,',','.') }}</option>
                                @endforeach
                            </select>
