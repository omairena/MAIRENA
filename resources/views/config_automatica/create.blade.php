        ...
        <div class="row">
            <div class="col-xl-12 order-xl-1">
                <div class="card">
                    ...
                    <div class="card-body">
                        <form method="post" action="{{ route('config_automatica.store') }}" autocomplete="off">
                            @csrf
                            ...
                            <div class="pl-lg-4">
                                <div class="form-group{{ $errors->has('idcaja') ? ' has-danger' : '' }}">
                                    ...
                                    <select class="form-control form-control-alternative" id="idcaja" name="idcaja" value="{{ old('idcaja') }}" required>
                                        @foreach($cajas as $caja)
                                            <option value="{{ $caja->idcaja }}">{{ $caja->codigo_unico }} - {{ $caja->nombre_caja }}</option>
                                        @endforeach
                                    </select>
                                ...
                                <div class="form-group{{ $errors->has('detalle_mensaje') ? ' has-danger' : '' }}">
                                    ...
                                    <input type="text" name="detalle_mensaje" id="detalle_mensaje" class="form-control form-control-alternative{{ $errors->has('detalle_mensaje') ? ' is-invalid' : '' }}" placeholder="{{ __('Detalle Mensaje para la Recepcion') }}" value="{{ 'RECEPCIONADAS AUTOMATICAMENTE POR CORREO ELECTRONICO' }}" required>
