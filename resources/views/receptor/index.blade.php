...
<div class="row">
        <div class="col-md-12">
            <div class="card">
                ...
                <div class="card-body">
                <form method="post" action="{{ route('filtro.receptor') }}" autocomplete="off">
    @csrf
        ...
        <div class="text-center">
            ...
            <div class="card ">
                <div class="card-header">
                    <div class="row">
                        <div class="col-8">
                            <h4 class="card-title">{{ __('Recepcion de Documentos') }}</h4>
                        </div>
                        <div class="col-4 text-right">
                            <div class="dropdown">
                                ...
                                <div class="dropdown-menu dropdown-menu-right dropdown-menu-arrow">
                                    <a href="{{ route('receptor.create') }}" class="dropdown-item">{{ __('Nueva Recepci贸n') }}</a>
                                    <a href="{{ route('receptor.consultar') }}" class="dropdown-item">{{ __('Consultar Documentos') }}</a>
                                    @if($new_recepcion > 0)
                                        <a href="{{ route('receptor.automatica') }}" class="dropdown-item">{{ __('Doc.x Aceptar') }}</a>
                                    @endif
                                    <a href="{{ route('receptor.recepcion') }}" class="dropdown-item">{{ __('Consultar Correo') }}</a>
                                </div>
                            </div>
                        ...
                        <table class="table tablesorter " id="ver_receptor_datatable">
                            <thead class=" text-primary">
                                ...
                                <th scope="col">{{ __('Detalle Mensaje') }}</th>
                                 <th scope="col">{{ __('Tipo Recp.') }}</th>
                                <th scope="col">{{ __('Estatus Hacienda') }}</th>
                                <th scope="col"></th>
                            </thead>
                            <tbody>
                                    ...
                                    <tr>
                                        ...
                                        <td>
                                        	<div class="dropdown">
                                                ...
                                                <div class="dropdown-menu dropdown-menu-right dropdown-menu-arrow">
                                                    <a class="dropdown-item" href="#" data-toggle="modal" data-target="#Haciendamodalreceptor" data-id="{{ $sale->idreceptor }}" id="clickModalReceptor">{{ __('Mensaje Hacienda') }}</a>
                                                    <a href="{{ url('donwload-xml-receptor-original', ['idsales' => $sale->idreceptor]) }}" class="dropdown-item">{{ __('Descargar XML Original') }}</a>
                                                    <a href="{{ url('donwload-xml-receptor', ['idsales' => $sale->idreceptor]) }}" class="dropdown-item">{{ __('Descargar XML->Envio') }}</a>
                                                    @if(!is_null($sale->xml_respuesta))
                                                    <a href="{{ url('donwload-xml-receptor-respuesta', ['idsales' => $sale->idreceptor]) }}" class="dropdown-item">{{ __('Descargar Respuesta MH XML') }}</a>
                                                    ...
                                                    <a class="dropdown-item" href="#" data-toggle="modal" data-target="#cambiarClasificacion" data-id="{{ $sale->idreceptor }}" id="clickcambiarClasif">{{ __('Cambiar Clasificacion') }}</a>
													@if(is_null($sale->estatus_hacienda))
                                                     <a href="{{ url('reenviar-docrecp', ['idsales' => $sale->idreceptor]) }}" class="dropdown-item">{{ __('Reenviar Documento') }}</a>
													  @endif
                                                      @if($sale->estatus_hacienda=='rechazado')
                                                     <a href="{{ url('reenviar-docrecp', ['idsales' => $sale->idreceptor]) }}" class="dropdown-item">{{ __('Reenviar Documento') }}</a>
													  @endif
...
@include('modals.cambiarClasificacion')
