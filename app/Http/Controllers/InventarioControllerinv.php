...
class InventarioController extends Controller
{
    	...
    	public function show($id)
    {
        ...
        $proveedores = Cliente::where('tipo_cliente', 2)->get();
        $productos = Productos::where('tipo_producto', 1)->paginate(8);
    	...
    	public function update(InventarioRequest $request, $id)
    {
        ...
        $datos = $request->except(['_token', '_method', 'cantidad_inventario','sales_item','costo_producto']);
        if ($datos['condicion_movimiento'] === '2') {
            $validator = Validator::make($datos, [
                'plazo_credito' => 'required|min:1|max:60',
            ]);
