...
class BoletasController extends Controller
{
    ...
    $fecha_desde = date('Y-m-01', strtotime('-3 months')); // Primer día del mes, 6 meses atrás
        
        $pedidos = Pedidos::where([
            ['idconfigfact', '=', Auth::user()->idconfigfact],
             ['tipo', 2],
             
             ['fecha_doc', '>=', $fecha_desde],
            ['fecha_doc', '<=', $fecha_hasta]
    	...
    	return view('boletas.index', ['pedidos' => $pedidos]);
    }
        ...
        public function create()
    {
        ...
        return view('boletas.create', ['productos' => $productos, 'contado' => $contado, 'cajas' => $cajas]);
    }

public function limpiarBoletas()
    {
        // Elimina los pedidos donde tipo sea 2 y estatus_doc sea 1
        Pedidos::where('tipo', 2)->where('estatus_doc', 1)->delete();

        // Redirige de vuelta a la lista de boletas con un mensaje de 谷xito
        return redirect()->route('boletas.index')->with('success', 'Boletas limpiadas exitosamente.');
    }
...
$pedido = Pedidos::create(
    [
        ...
        'tipo' => 2,
        'user'=> Auth::user()->id,
        'descripcion' => $datos['descripcion'], // Nuevo campo
        'marca' => $datos['marca'],               // Nuevo campo
        'modelo' => $datos['modelo'],             // Nuevo campo
        'serie' => $datos['serie'],               // Nuevo campo
        'factura' => $datos['factura'],           // Nuevo campo
        'num_servicio' => $datos['num_servicio'], // Nuevo campo
        'fecha_venta' => $datos['fecha_venta'],   // Nuevo campo
        'tiene_garantia' => $datos['tiene_garantia'], // Nuevo campo (1 o 0)
        'accesorios' => $datos['accesorios'],     // Nuevo campo
        'falla' => $datos['falla'],                 // Nuevo campo
        'observaciones' => $datos['observaciones']  // Ya existente
    ]
);
...
$consecutivo = DB::update('update consecutivos set numero_documento = '.$new.' where tipo_documento = '.$datos['tipo_documento'].' and idcaja = '.$datos['idcaja']);
        ...
        if(strstr($datos['sales_item'],',') ){
            ...
            return redirect()->route('boletas.edit', $pedido->idpedido);
        }else{
             if (!is_null($datos['idproducto_pos'])) {
        ...
        public function edit($id)
    {
        ...
        return view('boletas.edit', ['clientes'  => $clientes, 'productos' => $productos, 'pedido' => $pedido, 'pedidos_item' => $pedidos_item, 'usuario' => $usuario, 'cajas' => $cajas]);
    }
...
$cli=Cliente::where([
                    ['idconfigfact', '=', Auth::user()->idconfigfact],
                    ['nombre', '=', $datos['cliente_serch']],

                ])->get();
                

if (is_null($datos['boleta']) and !empty($datos['cliente_serch']) ){
...
$pedidos = Pedidos::where([
 ...
 if (!empty($datos['boleta']) and empty($datos['cliente_serch']) ){
    
    
$pedidos = Pedidos::where([
            ['idconfigfact', '=', Auth::user()->idconfigfact],
            ['tipo', 2],
            ['numero_documento', 'LIKE', '%' . $datos['boleta'] . '%']
        ])->orderBy('numero_documento', 'desc')->get();
        $cajas = Cajas::where([
            ['estatus', 1],
...
$pedidos = Pedidos::where([
        ...
        public function update(Request $request, Pedidos $model)
    {
        ...
        $estado= $datos['estado_boleta'];
        
       if ($estado == 4 || $estado == 3) {
           //dd($datos);
    // Si el estado es 4 o 3, asigna el estado de la boleta
    $estado_act = $datos['estado_boleta'];
} else {
    // En cualquier otro caso, asigna el estado 2
    $estado_act = 2;
}
        ...
        $pedido = Pedidos::find($datos['idpedido']);

    // Obtener los datos actuales de cierre
    $datos_cierre_actual = $pedido->datos_cierre;

    // Construir la nueva línea con la fecha y hora
    $nuevo_registro = now()->format('Y-m-d H:i:s') . " - " . $datos['datos_cierre'] . "\n"; // o usa el formato que prefieras

    // Concatenar el nuevo registro al existente
    $datos_cierre_actual .= $nuevo_registro;
        ...
        $act_pedido  = Pedidos::where('idpedido', $datos['idpedido'])->update([
                    ...
                    'datos_cierre' =>$datos_cierre_actual
        ]);

       

         $this->correoBoleta($request, $datos['idpedido']);

    return redirect()->route('boletas.index')->withStatus(__('Boleta Agregada Correctamente.'));
    }
public function correoBoleta(Request $request, $id)
    {
        app('App\Http\Controllers\ReportesController')->pdfBoleta($id);
        $pedido = Pedidos::find($id);
        $cliente = Cliente::find($pedido->idcliente);
        $configuracion = Configuracion::find($pedido->idconfigfact);
        $subject = 'Boleta #'.$pedido->numero_documento;
        $for = $cliente->email;
        ...
        try {
            ...
            Mail::send('mails.cotizacion',$datos, function($msj) use($subject,$for,$attach){
                $msj->from("facturacion@snsilvestre.com","Factura Electr車nica San Esteban");
                $msj->subject($subject);
                $msj->to($for);
                $msj->attach($attach);
            });
        ...
        return redirect()->route('boletas.index')->withStatus(__('Boleta Enviada Por Email Correctamente.'));
    }
        public function convertirPedido($id)
    {
        ...
        if (count($cajas) <= 0) {
            return redirect()->route('boletas.index')->withStatus(__('Verifique que la caja este abierta.'));
        }
        $p_credito = '00';
        $actividades = Actividad::where('idconfigfact', $pedido->idconfigfact)->get();
        if (count($actividades) <= 0) {
            return redirect()->route('boletas.index')->withStatus(__('Verifique que exista alguna actividad en la empresa.'));
        }
