...
function Crea_clave() {
    ...
    $documento = $this->xmlCompleto['numeroFactura'];
    $cod_tipodoc = $this->xmlCompleto['tipoDocumento'];
    $emision = $this->xmlCompleto['fechaEmision']; // podría venir vacío

    // Si $emision está vacío, obtener fecha/hora actual en Costa Rica (America/Costa_Rica)
    if (empty($emision)) {
        $zone = new DateTimeZone('America/Costa_Rica');
        $now = new DateTime('now', $zone);
        // Formato deseado: 2025-12-27T10:35:58-06:00
        $emision = $now->format('Y-m-d\\TH:i:sP');
    }
...
//}
	...
	function Guardar($pclave)
	{
		...
		switch ($this->xmlCompleto['tipoDocumento']) {
			case '01':
				$rutaxml = "./XML/".$idconfigfact."/Facturas/Envio/".$pclave.".xml";
				$envcorreo=1;
			break;
			case '02':
				$rutaxml = "./XML/".$idconfigfact."/NotaDebito/Envio/".$pclave.".xml";
				$envcorreo=1;
			break;
			case '03':
				$rutaxml = "./XML/".$idconfigfact."/NotaCredito/Envio/".$pclave.".xml";
				$envcorreo=1;
			break;
			case '04':
				$rutaxml = "./XML/".$idconfigfact."/Tiquete/Envio/".$pclave.".xml";
				$envcorreo=1;
			break;
			case '08':
				$rutaxml = "./XML/".$idconfigfact."/FacturaCompra/Envio/".$pclave.".xml";
				$envcorreo=2;
			break;
			case '09':
				$rutaxml = "./XML/".$idconfigfact."/FacturaExportacion/Envio/".$pclave.".xml";
				$envcorreo=1;
			break;
            case '09':
				$rutaxml = "./XML/".$idconfigfact."/FacturaExportacion/Envio/".$pclave.".xml";
			...
			break;
            case '10':
				$rutaxml = "./XML/".$idconfigfact."/ReciboPago/Envio/".$pclave.".xml";
				$envcorreo=1;
			break;
		}
	...
	function Guardar_receptor($pclave)
	{
		...
		switch ($this->xmlCompleto['tipoDocumento']) {
			case '05':
				$rutaxml = "./XML/".$idconfigfact."/DocReceptor/Envio/aceptados/".$pclave."-".$consecutivo.".xml";
			break;
			case '06':
				$rutaxml = "./XML/".$idconfigfact."/DocReceptor/Envio/parcialAceptados/".$pclave."-".$consecutivo.".xml";
			break;
			case '07':
				$rutaxml = "./XML/".$idconfigfact."/DocReceptor/Envio/rechazados/".$pclave."-".$consecutivo.".xml";
			break;
		}
		...
		for ($i=0; $i < count($this->xmlCompleto['DetalleServicio']); $i++) {
            ...
            if (!empty($this->xmlCompleto['DetalleServicio'][$i]['Impuesto'])) {
                for ($impto=0; $impto < count($this->xmlCompleto['DetalleServicio'][$i]['Impuesto']); $impto++) {
                    ...
                    //En el caso del Recibo Electrónico de Pago se detalla el pago al comprobante original, indicando al menos el número de cuota.
	...
	function armar_xml($clave){
		...
		if ($this->xmlCompleto['tipoDocumento'] != '04' && $this->xmlCompleto['tipoDocumento'] != '03' && $this->xmlCompleto['tipoDocumento'] != '09') {
		...
		$facturacion->appendChild($xmlDoc->createElement("CodigoActividadReceptor", $this->xmlCompleto['CodigoActividadReceptor']));
        	...
        	$codigoActividadReceptor = $this->xmlCompleto['CodigoActividadReceptor'] ?? null;
        	
        	
        		
if ($codigoActividadReceptor !== null && !in_array($this->xmlCompleto['tipoDocumento'], ['04', '10', '08'], true)) {
     if ($this->xmlCompleto['tipoDocumento'] == '03'){
         //	DD($codigoActividadReceptor);
			 if( substr($this->xmlCompleto['InformacionReferencia']['Numero'], 29, 2)== '01' ){
    $facturacion->appendChild(
        $xmlDoc->createElement("CodigoActividadReceptor", $codigoActividadReceptor)
    );
			 }else{
    ...
    foreach ($montosAcumulados as $item) {
        ...
        if($this->xmlCompleto['tipoDocumento'] === '08' or $this->xmlCompleto['tipoDocumento'] === '03'){
			...
			switch ($this->xmlCompleto['tipoDocumento']) {
				case '01':
					$ruta = "./XML/".$this->xmlCompleto['idconfigfact']."/Facturas/Envio/".$this->Crea_clave().".xml";
					file_put_contents($ruta, $xml_save);
				break;
				case '02':
					$ruta = "./XML/".$this->xmlCompleto['idconfigfact']."/NotaDebito/Envio/".$this->Crea_clave().".xml";
					file_put_contents($ruta, $xml_save);
				break;
				case '03':
					$ruta = "./XML/".$this->xmlCompleto['idconfigfact']."/NotaCredito/Envio/".$this->Crea_clave().".xml";
					file_put_contents($ruta, $xml_save);
				break;
				case '04':
					$ruta = "./XML/".$this->xmlCompleto['idconfigfact']."/Tiquete/Envio/".$this->Crea_clave().".xml";
					file_put_contents($ruta, $xml_save);
				break;
				case '08':
					$ruta = "./XML/".$this->xmlCompleto['idconfigfact']."/FacturaCompra/Envio/".$this->Crea_clave().".xml";
					file_put_contents($ruta, $xml_save);
				break;
				case '09':
					$ruta = "./XML/".$this->xmlCompleto['idconfigfact']."/FacturaExportacion/Envio/".$this->Crea_clave().".xml";
					file_put_contents($ruta, $xml_save);
				break;
			}
		return base64_encode($xml_save);
	...
	function armar_xml_receptor($clave, $consecutivo){
		...
		switch ($this->xmlCompleto['tipoDocumento']) {
			case '05':
				$ruta = "./XML/".$this->xmlCompleto['idconfigfact']."/DocReceptor/Envio/aceptados/".$clave."-".$consecutivo.".xml";
				file_put_contents(public_path($ruta), $xml_save);
			break;
			case '06':
				$ruta = "./XML/".$this->xmlCompleto['idconfigfact']."/DocReceptor/Envio/parcialAceptados/".$clave."-".$consecutivo.".xml";
				file_put_contents(public_path($ruta), $xml_save);
			break;
			case '07':
				$ruta = "./XML/".$this->xmlCompleto['idconfigfact']."/DocReceptor/Envio/rechazados/".$clave."-".$consecutivo.".xml";
				file_put_contents(public_path($ruta), $xml_save);
			break;
		}
