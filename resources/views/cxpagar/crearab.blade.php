                    ...
                    <div class="card-header">
                        <div class="row align-items-center">
                            ...
                            <div class="col-4 text-right">
                                <a href="{{ route('cxpagar.show', ['idcxpagar' => $idcxpagar]) }}" class="btn btn-sm btn-success">{{ __('Atras') }}</a>
                            </div>
                        </div>
                    </div>
                    <div class="card-body">
                        @include('alerts.success')
                        <form method="post" action="{{ route('logcxpagar.store') }}" autocomplete="off" enctype="multipart/form-data">
                            @csrf
                            ...
                            <div class="pl-lg-4">
                                <div class="form-group{{ $errors->has('num_recibo_abono') ? ' has-danger' : '' }}">
                                    ...
                                    <input type="text" name="num_recibo_abono" id="input-num_recibo_abono" class="form-control form-control-alternative{{ $errors->has('num_recibo_abono') ? ' is-invalid' : '' }}" placeholder="{{ __('Número Recibo') }}" value="{{ old('num_recibo_abono') }}" readonly="">
                                ...
                                <div class="form-group{{ $errors->has('monto_abono') ? ' has-danger' : '' }}">
                                    ...
                                    <input type="text" step="any" name="monto_abono" id="input-monto_abono" class="form-control form-control-alternative{{ $errors->has('monto_abono') ? ' is-invalid' : '' }}" placeholder="{{ __('Monto Abonar') }}" value="{{ old('monto_abono') }}">
                                ...
                                <input type="text" name="idmovcxpagar" id="idmovcxpagar" value="{{ old('idmovcxpagar', $id) }}" hidden="true">
...
<script type="text/javascript">
    $(document).ready(function() {
        var APP_URL = {!! json_encode(url('/ajaxNumCuentaxp')) !!};
        traerNumCuentaxp(APP_URL);
