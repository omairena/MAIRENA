...
$sql = "select  COUNT(idsale)  cants, idconfigfact  from sales where fecha_creada BETWEEN '$nfecha' AND '$nfechaf' GROUP BY idconfigfact   "; // Consulta SQL
...
<h1>Consumo de Documentos por Cuenta</h1>
  <a class="foot-menu-item mbr-fonts-style display-7" href="/sistema/admin/Prestamo.php">Salir</a>
	<form role="form" method="post" action="index1.php" id="form">
	     <div class="row ">
       
          <label >Doc Emitidos:</label>
          <?php
            $categories = mysqli_query($con,"select count(idsale) as cant  from sales where fecha_creada BETWEEN '$nfecha' AND '$nfechaf' ");
...
var data = {
        ...
        <?php
         $categories = mysqli_query($con,"select * from configuracion where idconfigfact = $d->idconfigfact ");
                                            
                                            while ($cat=mysqli_fetch_array($categories)) { 
                                            $contr= $cat['nombre_emisor']; 
                                            } 
        
        ;?>
        
        "Mes: <?php echo $d->idconfigfact;?> <?php echo $contr;?>", 
