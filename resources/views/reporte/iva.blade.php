@extends('layouts.app', ['page' => __('Reporte de Cuentas por Cobrar'), 'pageSlug' => 'reportesCxc'])
@section('content')
<div class="row">
        <div class="col-md-12">
            <div class="card">
                ...
                <div class="card-body">   
                    <form method="post" action="{{ route('filtro.iva') }}" autocomplete="off" id="filtro_factura">
                    @csrf
                            <label class="form-control-label" for="input-fecha_desde">{{ __('Selecci贸n de Configuracion') }}</label>
                            <select class="form-control form-control-alternative" id="idconfigfact" name="idconfigfact" value="{{ old('idconfigfact') }}" required>
                             ...
                             <div class="form-group{{ $errors->has('actividad') ? ' has-danger' : '' }}" id="combo_actividad" style="display: none;">
                                    <label class="form-control-label" for="input-actividad">{{ __('C贸digo Actividad') }}</label>
                                    <select class="form-control form-control-alternative" id="actividad" name="actividad" value="{{ old('actividad') }}" >
                                    </select>
    ...
    $.ajax({  
        ...
        success: function(response) {  
            ...
            if (arreglo > 0) {  
                ...
                $('#actividad').find('option').remove(); // Eliminar opciones previas  
                
                // Añadir la opción "Todos" al inicio  
                $("#actividad").append('<option value="">Todos</option>');  

                // Ahora añadir las opciones obtenidas de la respuesta  
        ...
        $.ajax({  
            ...
            success: function(response) {  
                ...
                if (arreglo > 0) {  
                    ...
                    // Añadir las opciones obtenidas de la respuesta  
                    $(response['success']['codigo_actividad']).each(function(data) {  
                        $("#actividad").append('<option value="' + response['success']['codigo_actividad'][data].idcodigoactv + '">' + response['success']['codigo_actividad'][data].codigo_actividad + ' - ' + response['success']['codigo_actividad'][data].descripcion + '</option>');  
                    });  
                } else {  
