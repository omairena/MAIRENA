@extends('layouts.app', ['page' => __('Reporte de Abono CXC'), 'pageSlug' => 'reportesCxcAbono'])
@section('content')
<div class="row">
        <div class="col-md-12">
            <div class="card">
                ...
                <div class="card-body">   
                    <form method="post" action="{{ route('filtro.cxcabono') }}" autocomplete="off" id="filtro_factura">
                    @csrf
                            <label class="form-control-label" for="input-fecha_desde">{{ __('Selección de Cuenta por Cobrar') }}</label>
                            <select class="form-control form-control-alternative" id="idcxcobrar" name="idcxcobrar" value="{{ old('idcxcobrar') }}" required>
                                <option value="0">-- Seleccionar Todas las cuentas --</option>
                                @foreach($cxcobrar as $cxc) 
                                        <option value="{{ $cxc->idcxcobrar }}">{{ $cxc->idcxcobrar }} - {{ $cxc->cxcobrar_cli[0]->nombre }} - {{ number_format($cxc->saldo_cuenta,2,',','.') }}</option>
                                @endforeach
                            </select>
